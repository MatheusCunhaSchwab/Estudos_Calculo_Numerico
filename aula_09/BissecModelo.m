...
if abs(fa) < prec
   x = a;
   fx = fa;
   disp('como fa satisfaz a condição, podemos ignorar' );
    return;
 endif
 ...
 if abs(fb) < prec
   x = b;
   fx = fb;
   disp('como fb satisfaz a condição, podemos ignorar' );
   return;
 endif

 % Abandonar se nao houver garantia de raiz no intervalo
 if fa*fb > 0
   disp('como f(a)*f(b) > 0, não podemos garantir raizes nesse intervalo');
   return;
 endif
 ...
 for k = 1:MaxIter
  ...
  if abs(fx) < prec
    fprintf('valor encontrado na %d iteracao\n', k);
    break;
  endif
